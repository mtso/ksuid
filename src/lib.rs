...
#[derive(Debug, Clone, Copy, Ord, PartialOrd, Eq, PartialEq, Serialize, Deserialize, Hash)]
...
mod tests {
    ...
    fn test_hashable() {
        ...
        let ksuid1 = Ksuid::generate();
        let ksuid2 = Ksuid::generate();
        let mut s: HashSet<Ksuid> = HashSet::new();

        assert!(s.insert(ksuid1));
        assert!(s.insert(ksuid2));
        assert_eq!(s.len(), 2);
    }
}
